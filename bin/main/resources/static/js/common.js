...
function Tips(e, s) {
    e.wrap('<div class="tips-box"></div>').after('<span class="tips">' + s + "</span>"),
    setTimeout(function() {
        e.unwrap().siblings().remove(".tips"),
        e.focus()
    }, 4e3)
}
...
function zy_Countdown() {
    ...
    58 > zy_c_num && $(".zy_success").addClass("upmove"),
    0 >= zy_c_num && (zy_c_num = 60,
    $(".sendE2").hide(),
    $(".sendE").show(),
...
function show_send_sms(e) {
    ...
    0 >= e ? ($(".verify-tips.success").hide(500),
    $("#forgetpswMobileModal .send-verify,#verify_form .send-verify").html("重发验证码").css({
        background: "#5ECFBA"
    }).removeAttr("disabled"),
    sendOFF = !0,
    void 0) : (e--,
    setTimeout("show_send_sms(" + e + ")", 1e3),
    void 0)
}
    ...
    sendOFF && ($.ajax({
        ...
        success: function(e) {
            e.mobile ? "" != s ? $("#mobile_code_password_form_message").css({
                ...
                background: "#f2dede"
            }).html("") : ($(".verify-tips").removeClass("error").addClass("success").html(e.info).show(500),
            show_send_sms(60)) : "failure" == e.status && ("" != s ? $("#mobile_code_password_form_message").css({
                background: "#f2dede"
...
function zy_validate_Email(e, s) {
    ...
    if (e)
        ...
        var a = $(".zy_email .a>a")
          , t = s.split("@")[1];
        a.attr("href", hash[t]),
        $("#emailValidateEE span").html(s),
        (void 0 == hash[t] || null == hash[t]) && a.parent().hide(),
        o = !1
    }
    return o
}
function mobile_code_password_form_submit() {
    $.ajax({
        ...
        success: function(e) {
            e.mobile_code_f ? ($("#mobile_code_password-tips").html(e.mobile_code_f).show(500),
            $("#id_mobile_code_f").focus()) : "success" == e.status && (account = $("#id_account").val(),
...
function searchajax(e) {
    $.ajax({
        ...
        success: function(e) {
            ...
            0 == s.length)
                return $(".dd_listbox").hide(),
                void 0;
            var c = "";
            $(".careercraft").remove(),
            $.each(s, function(e, s) {
                c = '<div class="careercraft"><a target="_blank" href="/course/' + s.id + '/"><p class="pull-left course_info">' + s.name + '</p><p class="pull-right study_count">' + s.student_count + "人正在学习</p></a></div>",
                $("#course_name_ledg").append(c)
            }),
            $(".chcourse").remove(),
            $.each(o, function(e, s) {
                ...
                $("#incld_careers").append(c)
            }),
            $(".careerteacher").remove(),
            $.each(a, function(e, s) {
                c = '<div class="careerteacher"><a target="_blank" href="/u/' + s.teacher_id + '/" style="outline:none;"><img src="/uploads/' + s.image + '" class="dd_courseimg img-circle"><p class="courseteacher">' + s.name + '</p><p class="ls">|</p><p class="teachertt">' + s.title + '</p><p class="teacherdes">' + s.info + "</p></a></div>",
                $("#incld_teachers").append(c)
            }),
            $("#data-search").val() && (0 == o.length ? ($("#dd_csitem").hide(),
            $("#incld_careers").hide()) : ($("#dd_csitem").show(),
            $("#incld_careers").show()),
            0 == a.length ? ($("#dd_tutor").hide(),
            $("#incld_teachers").hide()) : ($("#dd_tutor").show(),
            $("#incld_teachers").show()),
            0 == s.length ? ($("#dd_craft").hide(),
            $("#course_name_ledg").hide()) : ($("#dd_craft").show(),
...
function Search() {
    ...
    $("#data-search").keyup(function(s) {
        ...
        return o && 13 == o.keyCode ? (e($(this).val()),
        void 0) : void 0
    })
}
function cutTimer(e, s) {
    window.setInterval(function() {
        var o = 0
          ...
          , t = 0
          , i = 0;
        e > 0 && (o = Math.floor(e / 86400),
        a = Math.floor(e / 3600) - 24 * o,
        t = Math.floor(e / 60) - 60 * 24 * o - 60 * a,
        i = Math.floor(e) - 60 * 60 * 24 * o - 60 * 60 * a - 60 * t),
        9 >= t && (t = "0" + t),
        9 >= i && (i = "0" + i),
        $(s + "#day_show").html(o + "天"),
        $(s + "#hour_show").html(a + "时"),
        $(s + "#minute_show").html(t + "分"),
        $(s + "#second_show").html(i + "秒"),
        e--
    }, 1e3)
}
function tab(e) {
...
function imgPopup(e) {
    ...
    o.src = e,
    o.className = "img",
    s += '<div id="imgzoom"><div id="imgzoom-image-ctn"><i class="imgzoom-close"></i>',
    s += '<img class="img" src="' + o.src + '" alt=""/>',
    s += "</div></div>",
    $("body").append(s);
    ...
    o.onload = function() {
        ...
        s = o.width,
        e = o.height,
        a.width() / a.height() >= s / e && (e > a.height() ? (a.find(".img").width(parseInt(a.height() * s / e)),
        a.find(".img").height(parseInt(a.height())),
        a.width(parseInt(a.height() * s / e)),
        a.height(parseInt(a.height()))) : (a.width(s),
        a.height(e))),
        a.css({
            left: "50%",
            top: "50%",
...
function yueKeLastStep(e) {
    ...
    $("#yueke-last-step .know").off().on("click", function() {
        "" == o.val() ? (tipsMsg = "请输入你的姓名",
        Tips(o, tipsMsg)) : "" == a.val() ? (tipsMsg = "请输入正确的手机号",
        Tips(a, tipsMsg)) : c.test(a.val()) ? (t.selected = 0 == t.val()) ? (tipsMsg = "未选择",
        Tips(t, tipsMsg)) : "" == i.val() ? (tipsMsg = "手机验证码输入错误，请重试",
        Tips(i, tipsMsg)) : ops_verify(e) : (tipsMsg = "请输入正确的手机号",
        Tips(a, tipsMsg))
    }),
    $("#ops-send-verify").on("click", function() {
        "" == a.val() ? (tipsMsg = "请输入正确的手机号",
        Tips(a, tipsMsg)) : c.test(a.val()) ? ops_send_sms() : c.test(a.val()) || (tipsMsg = "请输入正确的手机号",
        Tips(a, tipsMsg))
    })
}
function ovoService() {
...
function ops_verify(e) {
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function(e) {
            e.success ? ($("#yueke-last-step").modal("hide"),
            yueKeSuccess()) : (s = e.message,
            Tips(o, s))
        }
    })
}
...
function ops_send_sms() {
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function(s) {
            s.success ? Cuttime(a, 60) : (e = s.message,
            Tips(o, e))
        }
    })
}
function Cuttime(e, s) {
    var o = null;
    e.addClass("send").val("60s").attr("disabled", "disabled"),
    o = setInterval(function() {
        s--,
        e.val(s + "s"),
        0 >= s && (clearInterval(o),
        e.removeClass("send").val("重新发送").removeAttr("disabled"))
    }, 1e3)
}
...
$(".sendE a").click(function() {
    sendECount = setInterval(zy_Countdown, 1e3),
...
$("#btnForgetpsw").on("click", function() {
    captchaObjF3 && captchaObjF3.refresh(),
    OFF && (captcha(".newcaptcha"),
    OFF = !1),
...
$(document).ready(function() {
    $("#data-search").focus(function() {
        ...
        "" == keywd ? flayer_show() : searchajax(keywd)
    }).blur(function() {
        flayer_hide(500)
    }),
    $("#data-search").keyup(function() {
        return keywd = $("#data-search").val(),
        "" == keywd ? (listbox_hide(),
        flayer_show(),
        void 0) : (searchajax(keywd),
        void 0)
    }).blur(function() {
        return keywd = $("#data-search").val(),
        "" == keywd ? (listbox_hide(),
        void 0) : (listbox_hide(500),
        void 0)
    })
});
